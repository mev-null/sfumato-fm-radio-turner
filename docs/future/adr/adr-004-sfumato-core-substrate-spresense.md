...
- ステータス: Deferred (2026-08-31)(保留前は Proposed・方向性の記録)
...
- 関連: [../../philosophy.md](../../philosophy.md) / [../architecture-hardware.md](../architecture-hardware.md) / [../roadmap-hardware.md](../roadmap-hardware.md) / [adr-003-brain-implementation-approach.md](adr-003-brain-implementation-approach.md) / [adr-002-audio-output-sigma-delta-dac.md](adr-002-audio-output-sigma-delta-dac.md)

## コンテキスト

ビジョン図の **sfumato 核**(= 適応オーディオ「Sfumato DSP Core」+ コンテキスト認識「Brain」、[../architecture-hardware.md](../architecture-hardware.md)「全体ビジョン」)を、どの基板で実装するかを問う。現ビジョンは sfumato 核も Muscle=FPGA 内に置き、音を実際に触る適応 DSP を RTL、Brain は Register Bus 越しにパラメータだけ書く疎結合([adr-003](adr-003-brain-implementation-approach.md))としていた。ここに **「Sony Spresense を導入し、復調済みの音声に対して *環境音と調和する* 適応処理を行わせる」** という案が出たため、方向性を記録する。MVP の射程外(Phase 3 / 4)だが、信号 I/F・出力トポロジ・二本立ての原則に影響するため早めに置く。
...
### 将来への含み
- Phase 3 着手時に本 ADR を Proposed → Accepted へ更新し、候補1〜3 と出力トポロジ (a)/(b) を確定。確定後 [../architecture-hardware.md](../architecture-hardware.md) の sfumato 核の節と algo↔hdl 対応表へ反映する。
...
- 調達は急がない。Spresense は Phase 3 着手の直前まで買わない([../roadmap-hardware.md](../roadmap-hardware.md) ハードウェア調達方針)。
