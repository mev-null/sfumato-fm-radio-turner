...
algo(`src/algo/`)の FM ステレオ変復調モデルを実行し、出力(復調音声・解析グラフ)を確認する。方式・数値をモデル上で確かめ、品質ゲート(`make eval`)に載せる前の確認フロー。手早く回すだけなら `make run` でよい。
...
3. **シミュレーション実行**: `make run`
   - `src/algo/main.py` が送信 → 通信路(AWGN)→ 受信 → 復調を実行する。入力音源が無ければ自動でステレオ時報を生成する。
...
- サンプリングレート・帯域・偏移などの物理定数は `src/algo/settings.py` が単一の出どころ。条件を変えるときはここを編集する。
