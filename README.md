...
本システムは，入力を19kHz pilot信号，出力をNCOの位相特性し，その出力を再び入力側へ戻して誤差を計算する閉ループ（フィードバック制御系）である．
...
```
#### Gainの決定
PI制御を採用した場合，システムの安定性は，PゲインとIゲインに依存することになる，この２つの係数を，ループ帯域幅(```bandwidth```)と，```zeta```の二変数を用いて，以下のように決定した．
```
wn = 2 * np.pi * bandwidth  # wn: 自然角周波数
alpha = (2 * zeta * wn) / fs  # P-gain
beta = (wn * wn) / (fs * fs)  # I-gain
```
ここで，システムにおける二変数の意味は以下の通りである．
- ```wn```： システムがどれくらいのスピードで目標に追従するか（速さ）
- ```zeta```: 減衰比
### 1.5.2.2 システムの最適化
